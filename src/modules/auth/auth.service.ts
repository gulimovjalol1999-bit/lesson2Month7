...
export class AuthService {
  ...
  async login(loginDto: LoginDto) {
    ...
    return {message: "Otp sent your email"}
  }
  ...
  async verify(dto: VerifyDto) {
    ...
    if(foundedUser.otpTime && now > foundedUser.otpTime) throw new BadRequestException("otp expired!")

    await this.authRepo.update(foundedUser.id, {otp: "", otpTime: 0})

    const payload = {username: foundedUser.username, RolesUser: foundedUser.role}
